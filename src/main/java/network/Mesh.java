...
public class Mesh implements Serializable {
    ...
    public Mesh(NetworkConfig nc, TrafficConfig tc, PhysicalLayerConfig plc, OthersConfig oc, HashMap<String, HashMap<Double, Double>> modTrDistance, Util util) {
        ...
        for (NetworkConfig.ModulationConfig modConf : nc.getModulations()) {
        	Modulation mod = new Modulation(modConf.getName(), modConf.getMaxRange(), modConf.getM(), modConf.getSNR(), plc.getRateOfFEC(), linkList.get(0).getCore(0).getSlotSpectrumBand(), guarBand, physicalLayer.getPolarizationModes());
    ...
    public int maximumSlotsByLinks(){
    	int max = 0;
    	for (int i = 0; i < linkList.size(); i++) {
    		int num = linkList.get(i).getCore(0).getNumOfSlots();
    		if(num > max){
    			max = num;
    		}
    	}
