 ...
 * @author Alexandre
 */
public class PhysicalLayer2 implements Serializable {
	 ...
	 */
    public PhysicalLayer2(PhysicalLayerConfig plc, Mesh mesh, Util util){
        this.util = util;
	...
	public double computeSNRSegment1(Circuit circuit, Route route, int sourceNodeIndex, int destinationNodeIndex, Modulation modulation, int spectrumAssigned[], Circuit testCircuit, boolean addTestCircuit){
		...
		//System.out.println("\nPnli Normalized: "+ratioForDB(juraPnliNormalized));
...
//		}
	...
	public double computeModulationDistanceByBandwidth3(Modulation mod, double bitRate, Mesh mesh) {
		...
		for(int ns = 1; ns <= quantSpansPorEnlace; ns++){
			...
			double OSNR = computeSNRSegment(circuitCentral, circuitCentral.getRoute(), 0, circuitCentral.getRoute().getNodeList().size() - 1, circuitCentral.getModulation(), circuitCentral.getSpectrumAssigned(), null, false);
			double OSNRdB = PhysicalLayer.ratioForDB(OSNR);
...
*/	
	...
	public double computePowerSpectralDensityByBinarySearch(Circuit circuit, Route route, Modulation modulation, int spectrumAssigned[], double factorMult){
		...
		double slotBandwidth = route.getLinkList().firstElement().getCore(0).getSlotSpectrumBand();
	...
	public void testCamadaFisica() {
		...
		route.getLink(0).getCore(0).addCircuit(circuit1); //Verificar
		route.getLink(0).getCore(0).addCircuit(circuit2); //Verificar
		route.getLink(0).getCore(0).addCircuit(circuit3); //Verificar
