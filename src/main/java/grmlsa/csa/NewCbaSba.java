...
//New CBA-SBA
public class NewCbaSba implements CoreAndSpectrumAssignmentAlgorithmInterface{
	...
	public static final int LIMITEUTILIZACAO = 310;
	...
	public int coreAssignment(Circuit circuit) {
		...
		int utilizacaorelativa[] = new int[7];
		...
		for (int i = 6; i>=0; i--) {
			...
			utilizacaorelativa[i] = circuit.getSpectrumUtilizationAbsolut()[i]/circuit.getRoute().getHops();
		}
		
		for (int i = 6; i>=0; i--) {
			if ((coreVector[i] < coreVector[core]) && (utilizacaorelativa[i]<LIMITEUTILIZACAO)) {
				core = i;
			}
		}
