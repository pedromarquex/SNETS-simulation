...
public class DJK implements RoutingAlgorithmInterface {
    ...
    public void inforotas(Mesh mesh) {
    	int quantNos = mesh.getNodeList().size();
    	String[] cabecalho = {"Origem", "destino", "Distancia"}; 
    	List<String[]> linhas = new ArrayList<>();
    	Link link = null;
    	
    	
    	for(int i=1; i<=quantNos; i++) {
    		for (int j=1; j<=quantNos; j++) {
    			if (i != j) {
    				//System.out.println("teste2");
    				Route route = routesForAllPairs.get(Integer.toString(i) + DIV + Integer.toString(j));
    				
    				linhas.add(new String[]{Integer.toString(i),
    						Integer.toString(j), //Core1
    						Double.toString(route.getDistanceAllLinks())});
    			}
    		}
    	}
    	...
    	Writer writer = null;
        try {
        	writer = Files.newBufferedWriter(Paths.get("inforotas.csv"));
    	} catch (IOException e) {
    		e.printStackTrace();
    	}
